...
async def test_retrieval_request(attachments):
    ...
    assert chunk_from_image.page_image == 0
...
@pytest.mark.asyncio
@e2e_test(filenames=["alps_wiki.html", "test_image.png"])
async def test_separate_indexing_request(attachments):
    app = create_app(
        app_config=AppConfig(
            dial_url=middleware_host,
        )
    ...
    # TODO: Get indexing results from the attachments, when implemented

    indexing_json = json.loads(indexing_response.text)
    indexing_stage_names = _get_stage_names(indexing_json)
    ...
    retrieval_stage_names = _get_stage_names(json_response)
