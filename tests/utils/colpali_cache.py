...
class RecordingScoreProcessor:
    ...
    def score_multi_vector(
        ...
        output = self.real_processor.score_multi_vector(
...
class ReplayModel:
    ...
    def __init__(
        self,
        recorded_query_embeddings: List[Tensor],
        recorded_image_embeddings: List[Tensor],
    ):
...
class CachedColpaliModelResource(ColpaliModelResource):
    ...
    def __init__(
        self,
        colpali_config: ColpaliIndexConfig | None,
        use_cache: bool = True,
    ...
    def _load_recordings(
        ...
        Optional[List[Tensor]], Optional[List[Tensor]], Optional[List[Tensor]]
    ]:
        ...
        if query_cache_path.exists():
            with open(query_cache_path, "rb") as f:
                # Safe to use pickle for test cache files
        ...
        if scores_cache_path.exists():
            with open(scores_cache_path, "rb") as f:
                # Safe to use pickle for test cache files
                scores_data = pickle.load(f)  # noqa: S301
    ...
    def get_model_processor_device(self) -> tuple[Any, Any, torch.device]:
        ...
        cache_key = self._get_cache_key(
            self.config.model_name, str(self.config.model_type)
        )
    ...
    def _get_replay_objects(
        self, cache_key: str
    ) -> tuple[Any, Any, torch.device]:
        """Get replay objects for cached mode."""
        query_embeddings, image_embeddings, recorded_scores = (
            self._load_recordings(cache_key)
        )

        if (
            query_embeddings is not None
            and image_embeddings is not None
            and recorded_scores is not None
        ):
    ...
    def _get_recording_objects(
        ...
        self._recording_model = RecordingModel(
            real_model, cache_key, self.cache_dir
        )
        self._recording_processor = RecordingScoreProcessor(
            real_processor, cache_key, self.cache_dir
        )
        ...
        if self._recording_model:
            return {
                "query_embeddings": len(
                    self._recording_model.recorded_query_embeddings
                ),
                "image_embeddings": len(
                    self._recording_model.recorded_image_embeddings
                ),
    ...
    def get_recorded_scores(self) -> dict:
        ...
        if self._recording_processor:
            return {"scores": len(self._recording_processor.recorded_scores)}
        return {}
